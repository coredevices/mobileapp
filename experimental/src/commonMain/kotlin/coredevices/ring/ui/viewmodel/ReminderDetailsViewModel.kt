...
import coredevices.ring.agent.builtin_servlets.reminders.builtInReminderFromData
    ...
    fun deleteReminder() {
        viewModelScope.launch {
            if (_itemState.value is ItemState.Loaded) {
                builtInReminderFromData((_itemState.value as ItemState.Loaded).dbItem).let {
                    it.cancel()
                    onBack()
                }
            }
